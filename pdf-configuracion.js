...
// Versión corregida para plantillas limpias
// =====================================

async function cfgPdfCargarConfiguracion() {
...
async function cfgPdfAplicar(bytes, conIVA) {

    const configuracion =
        await cfgPdfCargarConfiguracion();
...
generarM3 = async function (presupuesto) {
    ...
    const bytes =
        await cfgPdfGenerarM3Original(
            presupuesto
        );

    return await cfgPdfAplicar(
        bytes,
        false
    );
};
...
generarM3IVA = async function (presupuesto) {
    ...
    const bytes =
        await cfgPdfGenerarM3IVAOriginal(
            presupuesto
        );
